...
4. Update root manifest at `commerce-apps-manifest/manifest.json` with app entry (id, name, description, iconName, companyName, domain, version, zip, sha256). Optionally add `requiredFeatureToggle` to gate installation on a platform feature toggle (license-gated apps). To be considered for featured placement in the commerce apps workspace, also add `featuredTagline`, `featuredLearnMoreUrl`, and `featuredImageName` (commit the image to `commerce-apps-manifest/featured-images/`). Do **not** set `isFeatured` — it is reserved for Salesforce curation.
