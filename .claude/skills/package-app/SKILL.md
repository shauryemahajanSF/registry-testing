...
> **Featured app fields (optional):** To make an app eligible for featured placement in the commerce apps workspace, add `featuredTagline`, `featuredLearnMoreUrl`, and `featuredImageName` (commit the image to `commerce-apps-manifest/featured-images/`), and optionally `badge` (`"new"` or `"popular"`). Add `featuredTagline` to `commerce-apps-manifest/translations/en-US.json` as well so it can be localized. **Never set `isFeatured`** — it is reserved for Salesforce curation and must not be included in a submission.
